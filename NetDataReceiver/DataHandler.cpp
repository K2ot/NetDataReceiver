...
void DataManager::PushReceivedDataToTCPHandler(const std::vector<unsigned char>& frame)
{
	uint8_t codeHead{ 0xFF };
	codeHead = getCodeHead(static_cast<uint8_t>(GlobalData::FrameCore::Header), frame);


	erResCode = static_cast<bool>(codeRes >> 4);

	if (!IsCheckSumValid(frame))
	{

		throw std::invalid_argument("Niewlasciwa sumakontrolna" + std::to_string(codeHead) + ' ' + std::to_string(codeRes));
	}
	...
	std::vector<unsigned char> data{ GetData(static_cast<uint8_t>(dataSize), frame) };
	...
	try
	{
		...
		case static_cast<uint8_t>(CodeTcp::Message):
			...
			{
				handler.get().Handle(erResCode, data);
				break;
			}
			break;
	...
	{
		std::count << "Pozor" << e.what();
		throw;
	}
	catch (const std::runtime_error& e)
	{
		tsc_MessageBox::Show("Pozor", e.what());
		throw;
	}
}
	...
	{
		{
			...
			conditionVariable.wait(lock, [this]() { return inComeData; });
			...
			expectedDataSize = GetLittleEndian(frame) + CHECKSUM;
			int receivedDataSize = dataQueue.size();
...

		{
			...
			PushReceivedDataToTCPHandler(frame);
